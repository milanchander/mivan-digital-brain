...
## Mivan Health Plan — Commercial UM

> ⚠️ VALIDATE: This node was graduated from an SME-interview draft. The workflow, criteria model, and UM-to-claims gap analysis are sound, but the specific operational figures in sections 1–7 (exact decision timeframes, the "40–60% auto-approve" rate, and which criteria set maps to which service line) are industry-typical values that require confirmation against Mivan's actual UM policy and vendor contracts before treating as authoritative.

## Summary

Utilization Management (UM) governs whether and how services are authorized before, during, and after care — **prior authorization**, **concurrent review**, and **retrospective review** — for Mivan **commercial** members. Clinical staff produce the decisions: nurses apply MCG/InterQual criteria and may approve, but only a physician (Medical Director) may issue an adverse determination. Approved authorizations are written to **AUTH-FILE**, which **MAUTHCK0** reads at claim adjudication time. Medicare Advantage and Medicaid UM is adjudicated in **MiFCT (TriZetto Facets)**, not MiCPS. The highest-value content here is the **Known Gaps — UM-to-Claims Boundary** section, which documents where a valid authorization still results in a denied claim.
...
**MAUTHCK0 date tolerance** — part of the AUTH-FILE-LAYOUT critical ghost node. Undocumented tolerance logic auto-approves claims falling within N days of auth expiry. VALIDATE: the value of N is unknown. **The plan does not currently know its own auth expiry behavior.**
...
- **L1 — Enterprise Context:** `knowledge/L1-enterprise/mivan-enterprise-context.md`
- **L3 — System Landscape** (AUTH-FILE, MAUTHCK0, MiFCT routing): `knowledge/L3-systems/mivan-system-landscape.md`
- **L5 — Business Rules** (appeals, timely filing, overpayment recovery): `knowledge/L5-business-rules/claims-business-rules.md`
- **Commercial claims domain:** `knowledge/L2-domain/commercial-claims.md`
- **Related ghost nodes:** `AUTH-FILE-LAYOUT` (L4 — auth record layout & MAUTHCK0 tolerance), `NCCI-TABLE-MANAGEMENT` (L4)
- **Source draft:** `knowledge/MEM/contributions/UTILIZATION-MANAGEMENT-2026-08-09.md`
